...
const playerMarketScale = 1.5;

const playerStartX = 192;
const playerStartY = 238;

const defaultReturnX = 240;
const defaultReturnY = 528;
const marketExitObjectName = 'player_crop_market_exit';
...
export class CropMarket extends Scene {
    ...
    create() {
        const map = this.make.tilemap({ key: 'cropMarketMap' });
        this.exitZones = [];

        this.add.image(0, 0, 'cropMarketImage')
            .setOrigin(0)
            .setDepth(0);
    ...
    private resizePlayerForMarket(): void {
        this.player.sprite.setScale(playerMarketScale);
    ...
    private loadExitInteractionZones(map: Phaser.Tilemaps.Tilemap): void {
        ...
        for (const tiledObject of interactionLayer?.objects ?? []) {
            ...
            if (objectName !== marketExitObjectName || width <= 0 || height <= 0) {
                continue;
            }
