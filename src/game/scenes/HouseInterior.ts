...
const playerInteriorScale = 1.5;

const playerStartX = 192;
const playerStartY = 238;

const defaultReturnX = 672;
const defaultReturnY = 496;
const houseExitObjectName = 'player_house_exit';
...
export class HouseInterior extends Scene {
    player: Player;
    ...
    create() {
        const map = this.make.tilemap({ key: 'houseInteriorMap' });
        this.exitZones = [];

        this.add.image(0, 0, 'houseInteriorImage')
            .setOrigin(0)
            .setDepth(0);
    ...
    private resizePlayerForInterior(): void {
        this.player.sprite.setScale(playerInteriorScale);
    ...
    private loadExitInteractionZones(map: Phaser.Tilemaps.Tilemap): void {
        ...
        for (const tiledObject of interactionLayer?.objects ?? []) {
            ...
            if (objectName !== houseExitObjectName || width <= 0 || height <= 0) {
                continue;
            }
    ...
    private setupKeys(): void {
        this.input.keyboard!.on('keydown-E', this.tryExitHouse, this);

        this.events.once('shutdown', () => {
            this.input.keyboard?.off('keydown-E', this.tryExitHouse, this);
        });
    }

    private tryExitHouse(): void {
